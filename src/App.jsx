...
export default function App() {
  ...
  const resultado = fallas.find(f => f.dispositivo === dispositivoSeleccionado && f.falla === fallaSeleccionada);
        ...
        <>
          <div>
            ...
            <select onChange={(e) => setDispositivoSeleccionado(e.target.value)}>
              ...
              {dispositivos.map((d, i) => <option key={i} value={d}>{d}</option>)}
            </select>
          </div>
          ...
          {dispositivoSeleccionado && (
            <div>
              ...
              <select onChange={(e) => setFallaSeleccionada(e.target.value)}>
                <option value="">Seleccione</option>
                {fallasFiltradas.map((f, i) => <option key={i} value={f.falla}>{f.falla}</option>)}
              </select>
            </div>
          )}

          {resultado && (
            <div style={{ backgroundColor: '#f5f5f5', padding: 20, marginTop: 20 }}>
